    ...
    return (
        <StatusIndicatorBox as={Link} to={`/server/${server.id}`} className={className} $status={stats?.status}>
            <div css={tw`flex items-center col-span-12 sm:col-span-5 lg:col-span-6`}>
                ...
                <div>
                    <p css={tw`text-lg text-white break-words`}>{server.name}</p>
                    {!!server.description &&
                    <p css={tw`text-sm text-white break-words`}>{server.description}</p>
                    }
                </div>
            </div>
            <div css={tw`hidden lg:col-span-2 lg:flex ml-4 justify-end h-full`}>
                <FontAwesomeIcon icon={faEthernet} css={tw`text-white`}/>
                <p css={tw`text-sm text-white ml-2`}>
                    {
                ...
                {(!stats || isSuspended) ?
                    isSuspended ?
                        ...
                        (server.isTransferring || server.status) ?
                            <div css={tw`flex-1 text-center`}>
                                <span css={tw`bg-neutral-500 rounded px-2 py-1 text-white text-xs`}>
                    ...
                    <React.Fragment>
                        ...
                        <div css={tw`flex-1 ml-4 sm:block hidden`}>
                            ...
                            <p css={tw`text-xs text-white text-center mt-1`}>of {memoryLimit}</p>
                        </div>
                        <div css={tw`flex-1 ml-4 sm:block hidden`}>
                            <div css={tw`flex justify-center`}>
                            ...
                            <p css={tw`text-xs text-white text-center mt-1`}>of {diskLimit}</p>
                        </div>
                    </React.Fragment>
                }
            </div>

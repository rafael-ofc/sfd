...
const EditScheduleModal = ({ schedule }: Props) => {
    ...
    return (
        ...
        >
            {({ isSubmitting }) => (
                <Form>
                    <h3 css={tw`text-2xl mb-6 text-white`}>{schedule ? 'Edit schedule' : 'Create new schedule'}</h3>
                    ...
                    <p css={tw`text-white text-xs mt-2`}>
                        The schedule system supports the use of Cronjob syntax when defining when tasks should begin
                        running. Use the fields above to specify when these tasks should begin running.
                    </p>
                    <div css={tw`mt-6 bg-neutral-700 border border-neutral-910 shadow-inner p-4 rounded`}>
                        <FormikSwitch
                            name={'onlyWhenOnline'}

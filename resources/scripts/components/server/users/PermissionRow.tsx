...
const Container = styled.label`
  ${tw`flex items-center border border-white rounded md:p-2 transition-colors duration-75`};
  text-transform: none;
  ...
  &:not(.disabled) {
      ...
      &:hover {
        ${tw`border-white bg-neutral-700`};
      }
  }
        ...
        <Container htmlFor={`permission_${permission}`} className={disabled ? 'disabled' : undefined}>
            <div css={tw`p-2`}>
                <Checkbox
                    ...
                    name={'permissions'}
                    value={permission}
                    css={tw`w-5 h-5 mr-2 border-white`}
                    disabled={disabled}
                />
            </div>
            <div css={tw`flex-1`}>
                ...
                <p css={tw`text-xs text-white mt-1`}>
